...
;
-- zona 48
update proc_pergerakan set asal_kecamatan = 'Mancak'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 48 and z.kota = p.asal_kota_kabupaten
            group by id
            order by random()
            limit 456*29/100
    )
;
update proc_pergerakan set asal_kecamatan = 'Cinangka'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 48 and z.kota = p.asal_kota_kabupaten
            group by id
...
;
-- zona 49
update proc_pergerakan set asal_kecamatan = 'Lebak Wangi'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 49 and z.kota = p.asal_kota_kabupaten
            group by id
            order by random()
            limit 468*21/100
    )
;
update proc_pergerakan set asal_kecamatan = 'Tanara'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 49 and z.kota = p.asal_kota_kabupaten
            group by id
...
;
update proc_pergerakan set asal_kecamatan = 'Carenang'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 49 and z.kota = p.asal_kota_kabupaten
            group by id
...
;
update proc_pergerakan set asal_kecamatan = 'Tirtayasa'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 49 and z.kota = p.asal_kota_kabupaten
            group by id
...
;
-- zona 50
update proc_pergerakan set asal_kecamatan = 'Jawilan'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 50 and z.kota = p.asal_kota_kabupaten
            group by id
            order by random()
            limit 432*27/100
    )
;
update proc_pergerakan set asal_kecamatan = 'Kopo'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 50 and z.kota = p.asal_kota_kabupaten
            group by id
...
;
update proc_pergerakan set asal_kecamatan = 'Pamarayan'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 50 and z.kota = p.asal_kota_kabupaten
            group by id
...
;
-- zona 51
update proc_pergerakan set asal_kecamatan = 'Petir'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 51 and z.kota = p.asal_kota_kabupaten
            group by id
            order by random()
            limit 529*28/100
    )
;
update proc_pergerakan set asal_kecamatan = 'Ciomas'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 51 and z.kota = p.asal_kota_kabupaten
            group by id
...
;
update proc_pergerakan set asal_kecamatan = 'Pabuaran'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 51 and z.kota = p.asal_kota_kabupaten
            group by id
...
;
-- zona 54
update proc_pergerakan set asal_kecamatan = 'Sukadiri'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 54 and z.kota = p.asal_kota_kabupaten
            group by id
            order by random()
            limit 333*15/100
    )
;
update proc_pergerakan set asal_kecamatan = 'Sepatan Timur'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 54 and z.kota = p.asal_kota_kabupaten
            group by id
...
;
update proc_pergerakan set asal_kecamatan = 'Sepatan'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 54 and z.kota = p.asal_kota_kabupaten
            group by id
...
;
-- zona 55
update proc_pergerakan set asal_kecamatan = 'Pasarkemis'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 55 and z.kota = p.asal_kota_kabupaten
            group by id
            order by random()
            limit 429*51/100
    )
;
update proc_pergerakan set asal_kecamatan = 'Sindangjaya'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 55 and z.kota = p.asal_kota_kabupaten
            group by id
...
;
update proc_pergerakan set asal_kecamatan = 'Kemiri'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 55 and z.kota = p.asal_kota_kabupaten
            group by id
...
-- zona 57 (NO NEED)
-- zona 58
update proc_pergerakan set asal_kecamatan = 'Solear'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 58 and z.kota = p.asal_kota_kabupaten
            group by id
            order by random()
            limit 707*31/100
    )
;
update proc_pergerakan set asal_kecamatan = 'Jambe'
    where id in (
        select p.id from proc_pergerakan p left join zonasi_kab z on z.kecamatan = p.asal_kecamatan
            where z.zona_id = 58 and z.kota = p.asal_kota_kabupaten
            group by id
